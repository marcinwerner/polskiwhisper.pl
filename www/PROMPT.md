...
- **c)** Agent czyta brief z `www/`, ma pełny dostęp do kodu macOS / dokumentacji repo, zadaje pytania (w tym 7. pytanie: gdzie ma trafić kod strony - w tym repo czy osobne repo), czeka na odpowiedzi, koduje
...
- (opcjonalnie) docs/ARCHITECTURE.md, docs/PRIVACY.md (jeśli istnieją)

Pomiń: PolskiWhisper/* (Swift code), .internal/* (gitignored), .git/*
...
g) Lokalizacja kodu strony: w tym repo (kod w www/site/, deploy z subdir Vercel) czy osobny repo polskiwhisper-landing? 
   - w tym repo: prościej, jeden git, ale kod strony web mieszka obok kodu aplikacji macOS
   - osobny repo: czystszy podział odpowiedzialności, ale dwa miejsca do utrzymania
...
- **a)** Otwarte nowe okno Claude Code Desktop z working directory na **`~/Documents/GitHub/polskiwhisper.pl/`** (folder repo apki - nowa sesja ma full access do kodu macOS/dokumentacji/briefu)
- **b)** Marcin ma przygotowane odpowiedzi na 7 pytań uściślających:
  ...
  - 7. dodatkowe: kod w tym repo (`www/site/`) czy osobny repo `polskiwhisper-landing`?
